...
def find_single_symbol(file):
    ...
    for l in lines:
        line_count += 1
        clean_line = l
        count = clean_line.count(symbol)
        if count % 2 != 0 and r'\"' not in clean_line and """'"'""" not in clean_line and '"""' not in clean_line:
            if print_file_name is False:
                print(file)
                print_file_name = True
            print("{:<10d}{}".format(line_count, clean_line), end="")
